...
auto_logger.propagate = False  # 전용 로그 → 루트(ui_engine/stdout) 중복 전파 방지
...
def html_escape(value):
    """HTML 특수문자 이스케이프. (일부 함수의 지역변수 'html'이 모듈 html을 가리는 충돌을 피하는 래퍼)"""
        ...
        if os.path.exists(file_path):
            try:
                with open(file_path, "r", encoding="utf-8") as f:
                    data = json.load(f)
                    if isinstance(data, list):
                        # 비-dict 항목(손상된 캐시)을 소스에서 제거해 모든 소비자(대시보드/탭/export) 보호
...
def validate_domain_list(domain_list: list):
    ...
    for domain in domain_list:
        ...
        for part in parts:
            ...
            for ch in part:
                # isalnum()은 유니코드 문자(한글/전각/위첨자)도 True → ASCII 영숫자/하이픈만 허용
                # (validate_ipv4_list와 동일하게 isascii() 가드. 비-ASCII 도메인은 방화벽에서 무효)
...
def load_firewall_env_values(path: str):
    ...
    try:
        # utf-8-sig: Notepad/VSCode가 붙인 UTF-8 BOM을 투명하게 제거(첫 키 손상 방지)
...
def validate_firewall_env_file(path: str, mode: str = "IP", selected_firewalls: list = None):
    ...
    elif mode in ("GROUP", "ANY"):
        # 그룹 조회는 IP/FQDN 양쪽을 읽으므로 특정 그룹 키를 강제하지 않고 연결정보만 검증
        common_required = []
    else:
        common_required = [
...
def validate_ipv4_list(ip_list: list):
    ...
    for ip in ip_list:
        ...
        valid = True
        for part in parts:
            # isdigit()는 일부 유니코드 숫자(예: 위첨자 ²³)에 True를 반환하지만 int()는
            # ValueError를 던진다. isascii()를 함께 검사해 ASCII 0-9만 통과시킨다.
            if not part.isascii() or not part.isdigit():
                valid = False
                break

            # leading-zero 거부(예: '01') → 같은 IP가 중복/불일치 host 객체로 생성되는 것 방지
...
def email_addr(obj):
    if not isinstance(obj, dict):
        return "None"
    local = str(obj.get("localAddress", "") or "").strip()
    domain = str(obj.get("domainAddress", "") or "").strip()
    if not local and not domain:
        return ""
    # 한쪽이 비어도 '@'를 유지해 누락(로컬/도메인)을 감추지 않는다(예: 'a@', '@b.com')
...
def timeline_value_matches_context(value, ctx):
    ...
    # 1~2자 짧은 키워드가 무관한 이벤트에 광범위하게 부분일치되는 것을 막는다
    # (이 substring 폴백은 SQLite 인덱스가 없을 때만 타는 경로).
    if raw and len(raw) >= 3 and raw in value.lower():
        return True
...
def normalize_timeline_xdr(d, ctx):
    ...
    # 다중 수신자 mailbox("a@x, b@y")는 콤마로 분리해 첫 주소로 식별하고, 각 주소를 개별 매치 토큰으로 넣는다
    raw_mailbox = str(row_data.get("mailbox", "") or "")
    mailbox_list = [a.strip() for a in raw_mailbox.split(",") if a.strip()]
    primary_mailbox = mailbox_list[0] if mailbox_list else raw_mailbox
    identity = resolve_identity_by_mailbox(primary_mailbox)
    ...
    event = make_timeline_event(
        ...
        (mailbox_list or [raw_mailbox]) + [identity.get("user_name"), identity.get("user_id")]
    )
    return event if ctx is None or timeline_event_matches(event, ctx) else None
...
def load_timeline_raw_event(cache_file, row_index):
    ...
    try:
        if str(cache_file).endswith(".jsonl"):
            # 인덱싱 시 load_jsonl(공백/비-dict 줄 스킵)로 row_index가 매겨지므로
            # 조회도 동일하게 load_jsonl을 재사용해 인덱스를 일치시킨다.
...
def query_timeline_index(keyword, sources=None):
    ...
    conn = sqlite3.connect(TIMELINE_INDEX_DB_PATH)
    try:
        init_timeline_index_db(conn)
        rows = conn.execute(sql, params).fetchall()
        file_count = conn.execute("SELECT COUNT(*) FROM timeline_files").fetchone()[0]
        indexed_mtimes = dict(conn.execute("SELECT path, mtime FROM timeline_files").fetchall())
    finally:
        conn.close()  # with 블록은 커밋만 하고 커넥션을 닫지 않으므로 명시적 close

    # 신선도 검사: 인덱스 이후 새로 쌓이거나 갱신된 캐시 파일이 있으면 경고
    # (검색은 인덱스 스냅샷 기준이므로 최신 이벤트가 누락될 수 있음을 사용자에게 알림)
    stale = False
    try:
        current_files = (
            list(iter_json_files(DETECTIONS_DAY_DIR, ".json"))
            + list(iter_json_files(EMAILS_DAY_DIR, ".json"))
            + list(iter_json_files(DLP_DAY_DIR, ".jsonl"))
        )
        for p in current_files:
            prev = indexed_mtimes.get(p)
            if prev is None or os.path.getmtime(p) > float(prev) + 1:
                stale = True
                break
    except Exception:
        stale = False
    ...
    if stale:
        message += "  ⚠ 인덱스가 최신 캐시보다 오래됨 — Config에서 재인덱싱 권장"
    return groups, {
        "message": message,
        "events": len(events),
        "groups": len(groups),
...
def index_timeline_cache_file(conn, cache_file, logical_source):
    ...
    try:
        ...
        # 한 파일의 stat 실패가 전체 재빌드를 중단시키지 않도록 로그 후 계속
        log.warning(f"Timeline index: os.stat failed for {cache_file}: {e}")
...
def rebuild_timeline_index(progress_cb=None):
    for path in (TIMELINE_INDEX_DB_PATH, f"{TIMELINE_INDEX_DB_PATH}-wal", f"{TIMELINE_INDEX_DB_PATH}-shm"):
        if os.path.exists(path):
            try:
                ...
                # 동시 검색이 파일을 잠그고 있으면 PermissionError 가능 → 중단 대신 로그
                log.warning(f"Timeline index: failed to remove {path}: {e}")
    ...
    try:
        ...
        conn.close()  # with 블록은 커밋만 하고 닫지 않으므로 명시적 close
...
def load_history_queries():
    ...
    # 파싱은 됐으나 최상위가 객체가 아니면(배열/문자열 등) raw.get에서 AttributeError 발생 방지
    if not isinstance(raw, dict):
        log.warning("[HISTORY QUERY] Queries.txt top-level is not an object")
        return []
...
def load_dept_map():
    ...
    try:
        ...
        # 조용히 빈 맵으로 퇴화하면 부서 분류가 전부 무너져도 추적이 안 되므로 로깅
        log.warning(f"load_dept_map failed for {path}: {e}")
    return result
...
class DlpClient:
    ...
    def close(self):
        # 세션의 keep-alive 커넥션 풀을 명시적으로 해제(매 DLP 새로고침마다 새로 생성되므로)
        try:
            self.session.close()
        except Exception:
            pass
    ...
    def _fetch_cf_logs(self, payload, timeout=None):
        ...
        # JSON 파싱을 먼저 시도한다. 유효 JSON이면 본문에 'Endpoint Protector'/'로그아웃' 같은
        # 부분문자열이 들어있어도 정상으로 취급한다(콘텐츠 키워드 기반 오탐 방지).
        try:
            result = r.json()
        except Exception as e:
            # 파싱 실패 시에만, 그리고 Content-Type이 JSON이 아닐 때만 로그인/메인 HTML로 판정
            if "json" not in content_type.lower() and self._looks_like_login_html(r.text):
                raise RuntimeError("DLP cf_log/list returned login/main HTML instead of JSON") from e
    ...
    def _merge_rows_unique(self, rows):
        ...
        for row in rows:
            if not isinstance(row, dict):
                continue
            # 0/"" 같은 falsy 값도 유효 id일 수 있으므로 truthiness(or 체인) 대신 '존재' 여부로 판정
            key = None
            for id_field in ("log_id", "id", "loclogid"):
                v = row.get(id_field)
    ...
    def fetch_daily_logs(self, date_str: str):
        try:
            ...
            # 0행뿐 아니라 '부분 응답'(filtered > 반환행수)도 페이징 폴백으로 보완한다.
            # (서버가 length=-1 요청을 잘라서 응답하면 일부 행이 조용히 누락되므로)
            if first_filtered > len(first_rows):
                log.warning(
                    f"DLP full-day fetch returned {len(first_rows)} rows but recordsFiltered={first_filtered}. "
...
class SophosFirewallClient:
    ...
    def _post_xml(self, reqxml: str) -> str:
        # (connect=5s, read=60s): 방화벽이 unreachable일 때 대량 생성이 대상마다 60초씩
        # 누적되지 않도록 연결 단계는 빠르게 실패시킨다.
        r = requests.post(
            self.url,
...
<Request>
  ...
  <Set operation="add">
    <IPHost>
      <Name>{self._xml_value(object_name)}</Name>
      <IPFamily>IPv4</IPFamily>
      {description_xml}
      <HostType>IP</HostType>
      <IPAddress>{self._xml_value(ip_address)}</IPAddress>{group_xml}
    </IPHost>
  </Set>
</Request>
""".strip()
...
<Request>
  ...
  <Set operation="add">
    <FQDNHost>
      <Name>{self._xml_value(object_name)}</Name>
      {desc_xml}
      <FQDN>{self._xml_value(domain)}</FQDN>{group_xml}
    </FQDNHost>
  </Set>
</Request>
""".strip()
...
class SophosClient:
    ...
    def refresh_detections_range(self, from_ts: str, to_ts: str):
        ...
        total_pages = None
        not_ready_count = 0
        max_not_ready = 60  # 202(미완료) 최대 대기 ~5분(5초 * 60). 초과 시 에러로 중단
        rate_limit_count = 0
        max_rate_limit = 30  # 429 최대 대기 ~5분(10초 * 30). 초과 시 에러로 중단
        ...
        while True:
            ...
            # 202(미완료)만 재시도한다. 진짜 400은 아래 raise_for_status로 보내 무한 루프를 막는다.
            if r.status_code == 202:
                not_ready_count += 1
                if not_ready_count > max_not_ready:
                    raise RuntimeError("detections results not ready (timeout)")
            ...
            if r.status_code == 429:
                ...
                if rate_limit_count > max_rate_limit:
                    raise RuntimeError("detections results rate-limited (timeout)")
            ...
            if total_pages is None:
                tp = pages.get("total")
                # total이 없거나 null/비정상이면 None으로 두고 '짧은 페이지' 기준으로 종료(데이터 누락 방지)
            ...
            # 종료: total_pages를 알면 그 기준, 모르면 한 페이지가 page_size 미만이면 마지막으로 간주
            if total_pages is not None:
                ...
                if len(items) < page_size:
                    log.info("Short page (< page_size) — assuming last page.")
                    break
    ...
    def run(self):
        try:
            ...
            if self.job_name == "DLP":
                ...
                # with: 세션 커넥션 풀을 새로고침 후 결정적으로 해제
                with DlpClient(progress_cb=lambda msg: self.progress.emit("DLP", msg)) as api:
                ...
                if self.job_name == "Detection":
                    ...
                    # 입력 날짜는 KST 기준 → KST 자정~자정 구간을 UTC로 변환해 전송한다.
                    # (수신 데이터는 KST 일자로 버킷팅되므로 UTC 'Z'로 보내면 9시간 어긋나
                    #  하루치 앞 9시간 누락 + 인접일 파일 덮어쓰기가 발생한다.)
                    _kst = timezone(timedelta(hours=9))
                    _start_dt = datetime.strptime(start, "%Y-%m-%d").replace(tzinfo=_kst)
                    _end_dt = datetime.strptime(end, "%Y-%m-%d").replace(
                        hour=23, minute=59, second=59, microsecond=999000, tzinfo=_kst
                    )
                    from_ts = _start_dt.astimezone(timezone.utc).strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3] + "Z"
                    to_ts = _end_dt.astimezone(timezone.utc).strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3] + "Z"
...
class LiveDiscoverWorker(QThread):
    ...
    def run(self):
        try:
            ...
            # osquery SQL에 그대로 보간되므로 작은따옴표를 '' 로 이스케이프해
            # 따옴표 포함 입력(예: O'Reilly)의 쿼리 깨짐 및 인젝션을 방지한다.
            keyword = self.program_name.strip().lower().replace("'", "''")
            ...
            if self.query_type == "Process":
                ...
                if raw_input.endswith("\\") or raw_input.endswith("/"):
                    # SQLite(osquery) 문자열 리터럴은 백슬래시를 이스케이프로 해석하지 않으므로
                    # 백슬래시 doubling을 하면 = 등치 비교가 단일 백슬래시 경로와 매칭되지 않는다.
                    # 작은따옴표만 이스케이프하고 백슬래시는 원본 그대로 둔다.
                ...
                # 2) 전체 경로 입력 → 해당 파일 확인
                    ...
                    # 위와 동일: 백슬래시 doubling 제거(= 등치 비교가 단일 백슬래시 경로와 매칭되도록)
            ...
            while waited < max_wait:
                if self.isInterruptionRequested():  # 종료 요청 시 폴링 중단(closeEvent의 wait가 빨리 반환)
                    return
...
class SophosLiveDiscoverClient:
    ...
    def get_results(self, run_id: str):
        ...
        # nextKey 커서로 전체 페이지를 누적(다른 Sophos 호출과 동일 패턴). 첫 페이지만 읽어
        # LIMIT 200 결과가 잘리던 문제 해결. seen_keys/상한으로 무한루프 방어.
        all_items = []
        page_from_key = None
        seen_keys = set()
        merged = {}
        for _ in range(200):
            ...
            r.raise_for_status()
            merged = r.json()
            all_items.extend(merged.get("items", []) or [])
            nk = (merged.get("pages") or {}).get("nextKey")
...
class SophosXdrQueryClient:
    ...
    def get_results(self, run_id: str, max_size: int = 1000):
        ...
        # nextKey 커서로 전체 페이지 누적(maxSize 한 페이지로 잘리던 문제 해결).
        # seen_keys/상한으로 무한루프 방어.
        all_items = []
        page_from_key = None
        seen_keys = set()
        merged = {}
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.auto_pending = []  # 대기 중 자동 새로고침 작업 큐(둘 다 대기 시 유실 방지)
    ...
    def apply_runtime_color_config(self, config, persist=False):
        ...
        for frame in self.findChildren(QFrame):
            ...
            if name in ("dashboardCard", "statCard", "scrollStatCard"):
                frame.setStyleSheet(self.card_style(name))
    ...
    def setup_report_font(self):
        try:
            ...
            for path in candidates:
                # endswith("malgun.ttf") 필터는 malgunbd.ttf 폴백을 영영 막으므로 제거
                if os.path.exists(path):
                    regular_path = path
                    break
    ...
    def is_dlp_blocked_row(self, row):
        ...
        # 저장된 raw event_id는 영문("Content Threat Blocked")이므로 영문+한글 키워드를 모두 본다.
        # (build_security_insight_metrics의 차단 판정과 동일하게 맞춰 리포트 내 수치 불일치 제거)
        event_name = str(row.get("event_id", "") or row.get("content_policy", "") or "").strip().lower()
        return any(x in event_name for x in ["block", "deny", "차단", "반려"])
    ...
    def build_dlp_dept_insight_lines(self, dlp_dept_rank, metrics):
        ...
        def is_messenger(target_name, dest_detail):
            ...
            # 호출부는 대상유형 문자열(예: "instant messaging")을 넘기므로 타입도 인식한다
            # (앱 이름 집합 MESSENGER_TARGETS만 검사하면 타입 문자열은 항상 미스 → 분류 누락)
            return (t in MESSENGER_TARGETS) or ("instant messaging" in t) or kw_match(d, MESSENGER_DEST_KW)
        ...
        for rank, item in enumerate(dlp_dept_rank[:5], 1):
            ...
            for group_row in top_dest_group_rows[:5]:
                # 실제 대상유형 문자열 목록으로 분류한다(target_text는 표시용이라 분류 불가).
                # 목록 중 하나라도 매칭되면 인정하고, 목록이 비면 dest_detail 키워드로 판정한다.
                target_type_names = group_row.get("target_type_names") or []
                dest_detail = str(group_row.get("dest_detail", "") or "")
                cnt = int(group_row.get("count", 0) or 0)

                tnames = target_type_names or [""]
                primary_target = target_type_names[0] if target_type_names else ""

                if is_ai(dest_detail):
                    ai_related = True
                    classified_dests.append(f"AI 서비스({dest_detail}) {cnt}건({top_share(cnt, total)}%)")
                elif any(is_messenger(t, dest_detail) for t in tnames):
                    messenger_related = True
                    classified_dests.append(f"메신저({dest_detail or primary_target}) {cnt}건({top_share(cnt, total)}%)")
                elif any(is_mail(t, dest_detail) for t in tnames):
                    mail_related = True
                    classified_dests.append(f"메일({dest_detail or primary_target}) {cnt}건({top_share(cnt, total)}%)")
                elif any(is_cloud(t, t, dest_detail) for t in tnames):
                    cloud_related = True
                    classified_dests.append(f"클라우드/파일공유({dest_detail or primary_target}) {cnt}건({top_share(cnt, total)}%)")
    ...
    def generate_security_report_v2(self):
        try:
            ...
            def mini_table(x, y_pos, headers, rows, col_widths, font_size=9):
                """헤더+행을 직접 그리는 소형 테이블 (페이지 하단 도달 시 자동 페이지 넘김)"""
                from reportlab.pdfbase.pdfmetrics import stringWidth
                row_h = 18

                def draw_header(yy):
                    c.setFillColorRGB(0.20, 0.35, 0.60)
                    c.rect(x, yy - row_h + 4, sum(col_widths), row_h, fill=1, stroke=0)
                    c.setFillGray(1)
                    c.setFont(rf, font_size)
                    ox = x
                    for h, cw in zip(headers, col_widths):
                        c.drawString(ox + 4, yy - 11, str(h))
                        ox += cw
                    c.setFillGray(0)
                    return yy - row_h
                ...
                for ri, row in enumerate(rows):
                    # 하단 여백(40) 아래로 내려가면 페이지를 넘기고 헤더를 다시 그린다(행 잘림 방지)
                    if y_pos - row_h < 40:
                        c.showPage()
                        PAGE_W, PAGE_H = A4
                        y_pos = PAGE_H - MARGIN
            ...
            # PDF에 embed한 임시 추이 그래프 PNG 정리(REPORT_DIR 무한 누적 방지)
            try:
    ...
    def apply_date_range(self):
        ...
        if start_qdate > end_qdate:
            QMessageBox.warning(self, "날짜 오류", "시작일이 종료일보다 늦습니다. 날짜 범위를 확인하세요.")
            return
        ...
        if current_tab == "Dashboard":
            ...
            if hasattr(self, "_refresh_detection"):
            ...
            for d in all_detections:
                ...
                # 대시보드와 동일한 공유 분류기 사용(sensor.type 직접 비교는 더 좁아 email 탐지 일부 누락)
                if get_detection_sensor_type(d) != "email":
                    continue
            ...
            if hasattr(self, "_refresh_detection_xdr"):
            ...
            if hasattr(self, "_refresh_email"):
                self._refresh_email()
    ...
    def _dispatch_auto_pending(self):
    ...
    def _on_refresh_ok(self, tab_name):
        ...
        try:
            ...
            # 표시(렌더) 단계 예외가 OK 슬롯을 중단시키지 않도록 격리. 'OK'였던 상태는 오류로 표기.
            log.exception("refresh post-processing failed")
            self.set_status(f"{tab_name} OK (표시 오류)", color="red")
            QMessageBox.warning(self, "표시 오류", f"{type(e).__name__}: {e}")
        finally:
            # 표시 단계에서 예외가 나도 대기 자동 작업은 반드시 디스패치(체인 정지 방지)
            self._dispatch_auto_pending()
    ...
    def _on_refresh_fail(self, tab_name, err):
        ...
        # 실패해도 대기 중 자동 작업은 이어서 실행(큐 유실 방지)
        self._dispatch_auto_pending()
                ...
                if kind == "sha256":
                    menu.addAction(
                        ...
                        lambda v=value: webbrowser.open(
                            f"https://www.virustotal.com/gui/search/{urllib.parse.quote(str(v), safe='')}"
                        ),
                    )
                elif kind == "ip":
                    menu.addAction(
                        ...
                        lambda v=value: webbrowser.open(
                            f"https://www.virustotal.com/gui/ip-address/{urllib.parse.quote(str(v), safe='')}"
                        ),
                    )

            # GPT Rule 분석 (각 테이블의 실제 Rule 컬럼만 대상으로; 하드코딩 col==6은 오탐)
            if column_map.get(col) == "rule":
...
"""
    ...
    def make_stat_card(self, title, value):
        ...
        frame.setStyleSheet(self.card_style("statCard"))
    ...
    def make_scroll_stat_card(self, title, value):
        ...
        frame.setStyleSheet(self.card_style("scrollStatCard"))
    ...
    def refresh_dashboard(self):
        ...
        for name, cnt in top_files:
            links.append(
                ...
                f"<a href='{html_escape(name)}' style='text-decoration:none; color:#111827; font-weight:700;'>{html_escape(name)}</a>"
                f"</td>"
        ...
        for sha, cnt in top_hash:
            ...
            links.append(
                f"<tr>"
                f"<td style='padding:2px 12px 2px 0;'>"
                f"<a href='{html_escape(sha)}' style='text-decoration:none; color:#111111; font-weight:600;'>{html_escape(short_sha)}</a>"
                f"</td>"
                f"<td align='right' style='padding:2px 0; color:#111827; font-weight:700;'>({cnt})</td>"
                f"</tr>"
            )
        ...
        # 시작일이 종료일보다 늦으면 date_list가 비어 이후 x_dates[0]/max() 등에서 크래시한다.
        if start_dt > end_dt:
            QMessageBox.warning(self, "날짜 오류", "시작일이 종료일보다 늦습니다. 날짜 범위를 확인하세요.")
            return
        ...
        # 이벤트 0건인 날은 비교맵에 키가 없으므로 .get(.,0)으로 0 처리한다.
        # (멤버십 검사로 막으면 어제5→오늘0 같은 -100% 변화가 'No Data'로 잘못 표시됨.
        #  calc_percent가 prev<=0이면 None을 반환하므로 '기준 없음' 케이스는 그대로 유지된다.)
...
"""
    ...
    def tab_detection(self):
        ...
        self.enable_context_menu(table, {4: "ip", 5: "ip", 7: "sha256", 8: "rule"})
            ...
            for d in data:
                ...
                for field, key in search_conditions:
                    if field == "ALL":
                        ...
                        matched = False; break
                    elif field == "Dept" and key not in str(dept_name).lower():
                        matched = False; break
                    elif field == "Username" and key not in str(user_name).lower():
                        matched = False; break
    ...
    def tab_detection_xdr(self):
        ...
        self.enable_context_menu(table, {1: "rule", 9: "ip", 11: "sha256"})
    ...
    def tab_email(self):
        ...
        def refresh():
            ...
            for m in data:
                ...
                # 비-dict 항목(손상된 캐시 등)에서 m.get() 전에 걸러낸다(가드를 맨 앞으로).
                if not isinstance(m, dict):
                    continue
    ...
    def tab_live_discover(self):
        ...
        # (currentTextChanged 연결은 아래 시그널 연결 블록에서 1회만 — 중복 연결 제거)
    ...
    def refresh_live_session_table(self):
        ...
        for session in sessions:
            ...
            try:
                _rc = int(session.get("result_count", 0) or 0)
            except (ValueError, TypeError):
                _rc = 0
            item_count.setData(Qt.DisplayRole, _rc)
    ...
    def open_live_discover_session_detail(self, row, column):
        ...
        created_at = str(session.get("created_at", ""))
        try:
            result_count = int(session.get("result_count", 0) or 0)
        except (ValueError, TypeError):
            result_count = 0
        query_mode = str(session.get("query_mode", "Live"))
        query_type = str(session.get("query_type", "Process"))
        ...
        table.verticalHeader().setVisible(False)
        table.setAlternatingRowColors(True)
        table.setSortingEnabled(False)  # 채우는 동안 정렬 비활성(행 뒤섞임 방지) → 채운 뒤 재활성
        table.setContextMenuPolicy(Qt.CustomContextMenu)
        ...
        table.setSortingEnabled(True)  # 채우기 완료 후 정렬 재활성
    ...
    def run_live_discover_query(self):
        ...
        # 실행 중 콤보/입력을 바꿔도 결과 정규화가 어긋나지 않도록 제출 시점 값을 워커에 보관.
        self.history_worker.endpoint_name = endpoint_name
    ...
    def _on_live_discover_ok(self, rows):
        ...
        # 실행 중 콤보/입력을 바꿔도 display_columns가 행 키와 어긋나지 않도록
        # 위젯을 다시 읽지 않고 워커가 제출 시점에 보관한 값을 사용한다.
        endpoint_name = getattr(self.live_worker, "endpoint_name", "")
        program_name = getattr(self.live_worker, "program_name", "")
        query_type = getattr(self.live_worker, "query_type", "")
    ...
    def _on_history_query_ok(self, rows):
        ...
        # 위젯을 다시 읽지 않고 제출 시점에 워커가 보관한 값을 사용한다.
        # (실행 중 사용자가 선택을 바꾸면 옛 결과가 새 쿼리 매핑으로 잘못 정규화됨)
        endpoint_name = getattr(self.history_worker, "endpoint_name", "")
        selected_query_name = getattr(self.history_worker, "query_name", "")
    ...
    def tab_dlp_file(self):
        ...
        def add_search_row(default_field="ALL", default_mode="포함", removable=True, first=False):
            ...
            if first:
                ...
                def remove_row():
                    # deleteLater는 비동기라 직후 refresh()에 이 행 필터가 그대로 남는다.
                    # 입력을 비우고 레이아웃에서 즉시 분리해 이번 refresh부터 제외되게 한다.
                    edit.clear()
                    row.setParent(None)
                    row.deleteLater()
                    refresh()
    ...
    def tab_timeline(self):
        ...
        def start_search():
            ...
            # 인덱스 재빌드가 같은 DB를 삭제/재생성 중이면 검색이 빈/부분 결과를 낼 수 있어 거부(상호배제)
            if getattr(self, "timeline_index_worker", None) and self.timeline_index_worker.isRunning():
                self.timeline_status_label.setText("인덱스 재빌드 중입니다. 완료 후 검색하세요.")
                return
    ...
    def run_firewall_group_query(self, firewall_name: str):
        ...
        env_ok, env_msg = validate_firewall_env_file(
            FIREWALL_ENV_PATH,
            mode="GROUP",  # 그룹 조회는 IP/FQDN 양쪽 → 특정 그룹키 강제 없이 연결정보만 검증
            selected_firewalls=[firewall_name]
        )
    ...
    def get_response_success_items(self):
        ...
        for row in range(self.response_result_table.rowCount()):
            ...
            # success는 전송(HTTP) 성공 여부일 뿐이라 방화벽 API 레벨 실패(HTTP 200 + 에러바디)도
            # True가 된다. 표/카운트와 동일하게 result == "SUCCESS"(status_code 200)만 성공으로 본다.
    ...
    def run_timeline_index(self):
        if getattr(self, "timeline_index_worker", None) and self.timeline_index_worker.isRunning():
            ...
            return
        # 검색 중이면 같은 DB를 삭제하게 되므로 거부(상호배제)
        if getattr(self, "timeline_worker", None) and self.timeline_worker.isRunning():
            QMessageBox.warning(self, "진행 중", "Timeline 검색이 진행 중입니다. 완료 후 다시 시도하세요.")
            return
        # 공유 상태표시/스피너를 쓰는 다른 작업과 직렬화
        if self.running:
            QMessageBox.warning(self, "진행 중", "이미 다른 작업이 실행 중입니다.")
            return
...
# ===== Detection row =====
        ...
        # 버튼은 inner row에 들어있으므로 stretch는 row에 적용해야 4개가 균등해진다
        # (folder_layout는 title+row 2개뿐이라 setStretch(2/3,..)는 무효였음)
        for i in range(4):
            row.setStretch(i, 1)
    ...
    def open_export_folder(self):
        try:
            ...
            QMessageBox.warning(self, "실패", "내보내기 폴더를 열 수 없습니다.")

    def open_report_folder(self):
        try:
            os.startfile(REPORT_DIR)
        except Exception:
            QMessageBox.warning(self, "실패", "보고서 폴더를 열 수 없습니다.")
    ...
    def create_report_trend_graph(self, detection_timeline, graph_path, font_name="Helvetica"):
        ...
        try:
            ...
            fig.clear()  # 아티스트/메모리 결정적 해제(반복 리포트 생성 시 누적 방지)
        return graph_path
        ...
        for dept_name, stat in dept_stats.items():
            ...
            for dest_name, group in sorted(
                ...
                top_dest_group_rows.append({
                    ...
                    # 대상유형 분류(메신저/메일/클라우드)에 사용할 실제 타입 문자열 목록.
                    # (target_text는 "타입 (건수)" 형태의 표시용 문자열이라 분류에 쓸 수 없음)
                    "target_type_names": [name for name, _ in group["target_types"].most_common(5)],
                })
    ...
    def build_security_insight_lines(self, metrics):
        ...
        # (DLP 차단 상위 부서 문장은 위 dlp_count>0 블록에서 1회만 추가한다 — 중복 블록 제거)
        return lines
    ...
    def export_detection_excel(self):
        ...
        try:
            ...
            log.exception("detection excel export failed")
            QMessageBox.critical(self, "Export Error", f"{type(e).__name__}: {e}\n대상 파일이 Excel에서 열려있는지 확인하세요.")
            return
    ...
    def export_detection_xdr_excel(self):
        ...
        try:
            ...
            log.exception("detection xdr excel export failed")
            QMessageBox.critical(self, "Export Error", f"{type(e).__name__}: {e}\n대상 파일이 Excel에서 열려있는지 확인하세요.")
            return
    ...
    def export_email_excel(self):
        ...
        try:
            ...
            log.exception("email excel export failed")
            QMessageBox.critical(self, "Export Error", f"{type(e).__name__}: {e}\n대상 파일이 Excel에서 열려있는지 확인하세요.")
            return
    ...
    def auto_refresh_detection(self):
        ...
        if self.running:
            if "Detection" not in self.auto_pending:
    ...
    def auto_refresh_email(self):
        ...
        if self.running:
            if "Email" not in self.auto_pending:
    ...
    def refresh_endpoint_manual(self):
        ...
        reload_all_data()  # 전역 데이터 재적재 (load_endpoints는 미정의였음)
    ...
    def refresh_org_manual(self):
        ...
        reload_all_data()  # 전역 데이터 재적재 (load_org는 미정의였음)
        if hasattr(self, "_refresh_org"):
            self._refresh_org()
        if hasattr(self, "_refresh_endpoint"):
            self._refresh_endpoint()
        if hasattr(self, "_refresh_dlp"):
    ...
    def closeEvent(self, event):
        # 작업 중인 QThread를 정리한 뒤 종료한다.
        # (미정리 시 Qt가 실행 중 스레드를 파괴하며 'QThread: Destroyed while thread is still running'로
        #  프로세스가 비정상 종료될 수 있음)
        for name in ("worker", "live_worker", "history_worker",
                     "firewall_group_worker", "response_worker",
                     "timeline_worker", "timeline_index_worker"):
            ...
            try:
                if w.isRunning():
                    ...
                    w.quit()
                    w.wait(8000)  # run() 종료까지 최대 8초 대기
            except RuntimeError:
                pass  # 이미 파괴된 C++ 객체는 무시
    ...
    def excepthook(exc_type, exc_value, exc_traceback):
        ...
        try:
            ...
            pass
        try:
            # append + 타임스탬프: 매 예외마다 덮어쓰지 않고 누적(로그 디렉토리에 기록)
            with open(os.path.join(LOG_DIR, "fatal_error.log"), "a", encoding="utf-8") as f:
