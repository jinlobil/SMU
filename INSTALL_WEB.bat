...
:install_smu
echo [1/6] Restoring and checking shared SMU sources...
python restore_smu_sources.py || goto failed

if not exist ".venv\Scripts\python.exe" (
    echo [2/6] Creating a private Python environment...
    python -m venv .venv || goto failed
) else (
    echo [2/6] Using the existing Python environment.
)

echo [3/6] Installing Python web packages...
".venv\Scripts\python.exe" -m pip install --disable-pip-version-check -r requirements-web.txt || goto failed

echo [4/6] Installing React packages...
call npm --prefix web_frontend install || goto failed

echo [5/6] Building the React interface...
call npm --prefix web_frontend run build || goto failed

echo [6/6] Checking the complete web application...
