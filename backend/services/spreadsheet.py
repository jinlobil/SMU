...
def _worksheet(rows: list[dict], columns: list[str], headers: dict[str, str]) -> bytes:
    ...
    for row_index, row_values in enumerate(values, 1):
        ...
        for column_index, value in enumerate(row_values, 1):
            cell = SubElement(row_node, "c", {"r": f"{_column_name(column_index)}{row_index}", "t": "inlineStr", "s": "1" if row_index == 1 else "2"})
...
def write_xlsx_workbook(path: Path, sheets: list[dict]) -> list[str]:
    ...
    for spec in sheets:
        ...
        prepared.append((safe_sheet_name(str(spec.get("name") or "Data"), used), rows, columns, dict(spec.get("headers") or {})))
...
<styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><fonts count="2"><font><sz val="11"/><name val="Calibri"/></font><font><b/><color rgb="FFFFFFFF"/><sz val="11"/><name val="Calibri"/></font></fonts><fills count="3"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill><fill><patternFill patternType="solid"><fgColor rgb="FF7C3AED"/><bgColor indexed="64"/></patternFill></fill></fills><borders count="1"><border/></borders><cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/></cellStyleXfs><cellXfs count="3"><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"/><xf numFmtId="0" fontId="1" fillId="2" borderId="0" xfId="0" applyFont="1" applyFill="1" applyAlignment="1"><alignment wrapText="1" vertical="top"/></xf><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0" applyAlignment="1"><alignment wrapText="1" vertical="top"/></xf></cellXfs></styleSheet>'''
    ...
    with ZipFile(path, "w", ZIP_DEFLATED) as archive:
        ...
        for index, (_name, rows, columns, headers) in enumerate(prepared, 1):
            archive.writestr(f"xl/worksheets/sheet{index}.xml", _worksheet(rows, columns, headers))
