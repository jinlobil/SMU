...
class LearnerService:
 ...
 def run(self,mode="incremental",sources=None,target_start="",target_end="",progress=lambda x:None):
  ...
  try:
   ...
   for source in sources:
    ...
    if mode!="full":
     ...
     if changed:
      progress(f"{source} · 재인덱싱/삭제 감지 · Source 통계 재구성")
      with self.store.connect() as d:
    ...
    progress(f"{source} · {len(rows):,}개 이벤트 분석")
    self._process_source(source,rows,target_start,target_end)
    with self.store.connect() as d:
   ...
   raise
 def _process_source(self,source,rows,target_start,target_end):
