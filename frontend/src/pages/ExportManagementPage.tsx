...
type Tab = "detections" | "xdr" | "firewall" | "inbound" | "outbound" | "dlp" | "firewallRules" | "report";
type FirewallConfig = { name: string; configured: boolean };

const exportJobKey = "smu.export.activeJob";
const tabOrder: Tab[] = ["detections", "xdr", "firewall", "inbound", "outbound", "dlp", "firewallRules", "report"];
const tabLabels: Record<Tab, string> = { detections: "Detection XLSX", xdr: "Email XDR XLSX", firewall: "Firewall Detection XLSX", inbound: "Inbound Mail XLSX", outbound: "Outbound Mail XLSX", dlp: "DLP File XLSX", firewallRules: "Firewall Rule XLSX", report: "Security Report PDF" };
...
export function ExportManagementPage() {
  ...
  const exportSpec = active !== "report" && active !== "firewallRules" ? schema.exports[active] : null;
  const selectedCount = active === "report" ? selectedSections.length : active === "firewallRules" ? selectedFirewalls.length : (selectedColumns[active] || []).length;
