...
- Sensitive Files와 Sensitive Sites 결과는 페이지당 50개로 조회하며 이전/다음 및 페이지 번호 이동을 지원합니다.
...
- 스케줄러의 Detection/Inbound/DLP/Outbound 수집은 한국 날짜가 바뀐 첫 실행에서 미확정 전일 데이터를 하루 전체 범위로 한 번 최종 수집한 뒤 당일 데이터를 최신화합니다. Fetcher는 솔루션별 마지막 확정 날짜를 `runtime/fetcher/jobs.db`에 저장하므로 같은 전일을 매 주기 반복 수집하지 않으며, 중단 기간의 미확정 날짜는 오래된 날짜부터 복구합니다. Endpoint/Organization/User는 날짜 데이터가 아닌 현재 상태 Snapshot으로 계속 수집합니다.
