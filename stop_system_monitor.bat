...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$items=Get-CimInstance Win32_Process; foreach($item in $items){if($item.CommandLine -match 'system_monitor[.]watchdog'){Stop-Process -Id $item.ProcessId -Force -ErrorAction SilentlyContinue}}; Start-Sleep -Milliseconds 500; foreach($item in $items){if($item.CommandLine -match 'system_monitor[.]collector' -or $item.CommandLine -match 'system_monitor[.]fetcher' -or $item.CommandLine -match 'system_monitor[.]indexer' -or $item.CommandLine -match 'system_monitor[.]laborer'){Stop-Process -Id $item.ProcessId -Force -ErrorAction SilentlyContinue}}"
if errorlevel 1 goto failed

echo Watchdog, Collector, Fetcher, Indexer, and Laborer processes stopped.
exit /b 0
