...
def test_machine_learning_reuses_existing_smu_ui_patterns():
    ui=Path("frontend/src/pages/MachineLearningPage.tsx").read_text(encoding="utf-8")
    for shared in ('className="source-filters"','<DateRange','className="condition-list"','className="primary-action"','className="pagination"','className="config-card learner-finding"','className="detail-modal"','className="timeline-empty"'):
        assert shared in ui
