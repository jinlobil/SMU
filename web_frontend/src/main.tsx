...
function App(){
  ...
  return <div className="app-shell">
    <motion.aside animate={{width:collapsed?82:264}} transition={{type:'spring',stiffness:420,damping:36}} className="sidebar">
      <div className="brand"><div className="brand-mark"><Shield size={22}/></div>{!collapsed&&<motion.div initial={{opacity:0}} animate={{opacity:1}}><strong>SMU</strong><span>Security Center</span></motion.div>}</div>
      <nav>{groups.map(group=><div className="nav-group" key={group}>{!collapsed&&<p>{group}</p>}{pages.filter(p=>p.group===group).map(item=>{const Icon=item.icon;return <button title={item.label} key={item.id} onClick={()=>setActive(item.id)} className={active===item.id?'active':''}><Icon size={19}/>{!collapsed&&<span>{item.label}</span>}{active===item.id&&<motion.i layoutId="nav-active"/>}</button>})}</div>)}</nav>
    ...
    <main>
      ...
      <AnimatePresence mode="wait"><motion.section key={active} initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-5}} transition={{duration:.2,ease:[.22,1,.36,1]}} className="content">
    ...
    <AnimatePresence>{notice&&<motion.div className="toast" initial={{opacity:0,y:20,scale:.96}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:10}}><Shield size={18}/><span>{notice}</span><button onClick={()=>setNotice(null)}><X size={16}/></button></motion.div>}</AnimatePresence>
  </div>
}
...
function Dashboard({start,end,refresh}:{start:string,end:string,refresh:number}){
 ...
 <div className="metrics">{cards.map(([label,value,tone],i)=><motion.article key={label} initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} transition={{delay:i*.045}}><span className={`metric-icon ${tone}`}><Activity/></span><div><p>{label}</p><strong>{Number(value).toLocaleString()}</strong><small>selected period</small></div></motion.article>)}</div>
 <div className="dashboard-grid"><article className="panel chart"><PanelTitle title="Threat activity" subtitle="선택 기간 이벤트 분포"/><div className="bars">{cards.map(([label,value,tone])=><div key={label}><span>{label}</span><div><motion.i className={String(tone)} initial={{width:0}} animate={{width:`${Math.max(Number(value)/max*100,2)}%`}} transition={{duration:.7,ease:[.22,1,.36,1]}}/></div><b>{Number(value).toLocaleString()}</b></div>)}</div></article><article className="panel posture"><PanelTitle title="Security posture" subtitle="실시간 로컬 인덱스"/><div className="score"><div><strong>92</strong><span>/100</span></div></div><h3>Excellent posture</h3><p>중요 이벤트를 계속 모니터링하고 있습니다.</p></article></div></>
}
