...
`core` 또는 `modules` 폴더가 다운로드에 없다면 설치기가
`web_source_bundle`에 포함된 검증된 원본을 같은 위치에 자동 복원합니다.
해당 공유 모듈의 import와 기능을 제거하지 않으며, 기존 파일이 있으면 절대로
덮어쓰지 않습니다. 설치 마지막 6단계에서 공유 storage, DLP formatter, theme,
FastAPI 앱과 필수 API 경로를 전부 검사한 뒤 서버를 시작합니다.
...
현재 수정본은 실행 로그 첫 부분에 `Release: 2026.07.21.3`을 표시합니다.
