...
이전 로그에 `No module named 'core'` 또는 `Required SMU directory is missing:
...\core`가 나왔다면 이전 웹 백엔드가 데스크톱 폴더에 의존했던 문제입니다.
새 웹 백엔드는 필요한 경로와 캐시 판독 기능을 `web_backend` 안에 포함하므로
`core`나 `modules` 폴더가 없어도 시작합니다. 설치 마지막 5단계에서 독립형
storage, theme, FastAPI 앱과 필수 API 경로를 전부 검사한 뒤 서버를 시작합니다.
...
현재 수정본은 실행 로그 첫 부분에 `Release: 2026.07.21.2`를 표시합니다.
이 문구가 없고 `ensure_local_package` 또는 `Required SMU directory is missing`
오류가 나오면 새 수정본이 아니라 이전 run_web.py를 실행하고 있는 것입니다.
